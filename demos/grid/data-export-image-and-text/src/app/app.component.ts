		...
		for (let i = 0; i < rows.length; i++) {
			...
			for (let j = 0; j < exportColumns.length; j++) {
				...
				if (column.dataField === 'Employee') {
					let value = 'https://htmlelements.com/demos/images/phonebook/' + dataSource[i].firstName.toLowerCase() + '.png';
	...
	columns = [
	    {
			label: 'Employee', cardSpan: true, cardHeight: 3, dataField: 'Employee', width: 250, cellsVerticalAlign: 'middle', verticalAlign: 'middle', align: 'center', cellsAlign: 'center', formatFunction(settings: any) {
				settings.template = '<img height="50" width="50" src="' + 'https://htmlelements.com/demos/images/phonebook/' + settings.row.data.firstName.toLowerCase() + '.png"/>' + '<span>' + settings.row.data.firstName + ' ' + settings.row.data.lastName + '</span>';
			}
		},
