...
@Component({
  selector: 'app-root',
  imports: [GridModule, ButtonModule, InputModule, PasswordInputModule],
  standalone: true,
...
export class AppComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    // Set up password input event
    if (this.passwordInput && this.passwordInput.nativeElement) {
      this.passwordInput.nativeElement.addEventListener('input', (event: any) => {
        const value = event.target.value;
        this.dataExport = {
          protectSheet: value.length > 0 ? { password: value } : true
        };
        // Update grid's dataExport property
        if (this.grid) {
          this.grid.dataExport = this.dataExport;
        }
      });
    }
  }
