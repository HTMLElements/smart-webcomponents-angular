...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [ ChartModule, DropDownListModule, SliderModule ],
...
export class AppComponent implements AfterViewInit, OnInit {
    ...
    init(): void {
        ...
        that.dropdownlist2.addEventListener('change', function () {
            that.chart.colorScheme = that.dropdownlist2.selectedValues[0];
        } as EventListenerOrEventListenerObject);
        that.dropdownlist.addEventListener('change', function () {
            that.chart.seriesGroups[0].type = that.dropdownlist.selectedValues[0];
            that.chart.update();
        } as EventListenerOrEventListenerObject);
    }
}