...
export class AppComponent implements AfterViewInit, OnInit {	
	...
	@ViewChild('collapsible', { read: CheckBoxComponent, static: false }) collapsible: CheckBoxComponent;
	@ViewChild('icons', { read: CheckBoxComponent, static: false }) icons: CheckBoxComponent;
	@ViewChild('titleField', { read: DropDownListComponent, static: false }) titleField: DropDownListComponent;
	@ViewChild('horizontal', { read: RadioButtonComponent, static: false }) horizontal: RadioButtonComponent;
	@ViewChild('vertical', { read: RadioButtonComponent, static: false }) vertical: RadioButtonComponent;
	@ViewChild('crop', { read: RadioButtonComponent, static: false }) crop: RadioButtonComponent;
	@ViewChild('fit', { read: RadioButtonComponent, static: false }) fit: RadioButtonComponent;
	...
	init(): void {
	    ...
	    const that = this;
        const sampleData = [],
         firstNames = ['Andrew', 'Nancy', 'Shelley', 'Regina', 'Yoshi', 'Antoni', 'Mayumi', 'Ian', 'Peter', 'Lars', 'Petra', 'Martin', 'Sven', 'Elio', 'Beate', 'Cheryl', 'Michael', 'Guylene'], lastNames = ['Fuller', 'Davolio', 'Burke', 'Murphy', 'Nagase', 'Saavedra', 'Ohno', 'Devling', 'Wilson', 'Peterson', 'Winkler', 'Bein', 'Petersen', 'Rossi', 'Vileid', 'Saylor', 'Bjorn', 'Nodier'], petNames = ['Sam', 'Bob', 'Lucky', 'Tommy', 'Charlie', 'Olliver', 'Mixie', 'Fluffy', 'Acorn', 'Beak'], productNames = ['Black Tea', 'Green Tea', 'Caffe Espresso', 'Doubleshot Espresso', 'Caffe Latte', 'White Chocolate Mocha', 'Cramel Latte', 'Caffe Americano', 'Cappuccino', 'Espresso Truffle', 'Espresso con Panna', 'Peppermint Mocha Twist'], contactImages = ['andrew.png', 'anne.png', 'avril.jpeg', 'janet.png', 'johanna.jpeg', 'johnny.jpeg', 'laura.png', 'margaret.png', 'Maria.jpeg', 'mark.jpeg', 'maya.jpeg', 'michael.png', 'monica.jpeg', 'nancy.png', 'robert.png', 'steven.jpeg', 'steven.png', 'toni.jpeg'];
         ...
         for (let i = 0; i < 20; i++) {
            ...
            for (let i = 0; i < maxPhotos; i++) {
                row.travelPhotos.push(`../../../images/travel/${Math.floor(Math.random() * 36) + 1}.jpg`);
                row.contactPhotos.push(`../../../images/phonebook/${contactImages[Math.floor(Math.random() * contactImages.length)]}`);
            }
        ...
        that.cardview.columns = [
            ...
            { label: 'Travel photos', dataField: 'travelPhotos', icon: 'photo', formatFunction: that.photoFormatFunction, image: true },
            { label: 'Contact photos', dataField: 'contactPhotos', icon: 'photo', formatFunction: that.photoFormatFunction, image: true }
        ];
