...
export function generateGanttDataSource(count) {
	const letters = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'],
		firstNames =
			['Andrew', 'Nancy', 'Shelley', 'Regina', 'Yoshi', 'Antoni', 'Mayumi', 'Ian', 'Peter', 'Lars', 'Petra', 'Martin', 'Sven', 'Elio', 'Beate', 'Cheryl', 'Michael', 'Guylene'],
		lastNames =
			['Fuller', 'Davolio', 'Burke', 'Murphy', 'Nagase', 'Saavedra', 'Ohno', 'Devling', 'Wilson', 'Peterson', 'Winkler', 'Bein', 'Petersen', 'Rossi', 'Vileid', 'Saylor', 'Bjorn', 'Nodier'],
		customers = ['Kronbergs Flagship', 'Vance Refrigeration', 'Walmart Inc.', 'Avangard Innovations', 'Whole Foods Inc.'],
		date = new Date();
	...
	function getRouteNumber() {
		...
		for (let i = 0; i < 5; i += 1) {
			routeNumber += letters[Math.round((Math.random() * 100) % (letters.length - 1))];
		}

		return 'RO_' + routeNumber.toUpperCase()
	}

	function getTicket(type, dateStart, minutes) {
		...
		switch (type) {
			case 'preRoute':
				dataObj['preRouteStartTime'] = `${startDate.getHours()}:${startDate.getMinutes()}`;
				dataObj['preRouteEndTime'] = `${endDate.getHours()}:${endDate.getMinutes()}`;
				break;
			case 'lunchTime':
				dataObj['lunchTimeStart'] = `${startDate.getHours()}:${startDate.getMinutes()}`;
				dataObj['lunchTimeEnd'] = `${endDate.getHours()}:${endDate.getMinutes()}`;
				break;
			case 'returnYard':
				dataObj['returnYardStart'] = `${startDate.getHours()}:${startDate.getMinutes()}`;
				dataObj['returnYardEnd'] = `${endDate.getHours()}:${endDate.getMinutes()}`;
				break;
			case 'returnToYard':
				dataObj['returnToYardStart'] = `${startDate.getHours()}:${startDate.getMinutes()}`;
				dataObj['returnToYardEnd'] = `${endDate.getHours()}:${endDate.getMinutes()}`;
				break;
			case 'postRoute':
				dataObj['postRouteStartTime'] = `${startDate.getHours()}:${startDate.getMinutes()}`;
				dataObj['postRouteEndTime'] = `${endDate.getHours()}:${endDate.getMinutes()}`;
				break;
		}
	...
	function getTickets(count = 10, completedTickets) {
		let tickets = [],
			random = Math.random(), lunchBreak, returnYard;
		const startDate = new Date(),
			loadTypes = ['S/O', 'E/R', 'DEL'],
			containerSizes = ['10O', '20C', '30O', '40C', ''],
			completionStatuses = ['to_DS', 'at_DS', 'from_DS', 'to_YD', 'at_YD', 'from_YD', 'to_CY', 'at_CY', 'from_CY', 'to_C', 'at_C', 'from_C'],
			transmissionStatuses = ['transmit_issue', 'transmit_ok', 'transmit_going', 'transmit_failed', 'recall', 'recall_ok', 'recall_fail', 'no_status'],
			orderIds = [];
		let activeTicketIndex = completedTickets || 0,
			minutes, endDate, serviceWindowStart, serviceWindowEnd;

		startDate.setHours(0, 0, 0, 0);

		const preRoute = getTicket('preRoute', startDate, minutes = Math.max(30, Math.round((Math.random() * 100) % 45)));

		tickets.push(preRoute);

		startDate.setMinutes(startDate.getMinutes() + minutes);

		for (let i = 0; i < count; i += 1) {
			let id = Math.round(random * 10000);

			startDate.setHours(startDate.getHours() + Math.round((Math.random() * 10) % 3), Math.round((Math.random() * 100) % 59), 0, 0);
			endDate = new Date(startDate);
			endDate.setHours(startDate.getHours() + Math.round((Math.random() * 10) % 2), startDate.getMinutes() + Math.max(30, Math.round((Math.random() * 100) % 59)), 0, 0);
			serviceWindowStart = new Date(startDate);
			serviceWindowEnd = new Date(endDate);
			serviceWindowStart.setHours(startDate.getHours() - Math.round((Math.random() * 10) % 2), startDate.getMinutes() + Math.max(30, Math.round((Math.random() * 100) % 59)), 0, 0);
			serviceWindowEnd.setHours(startDate.getHours() + Math.round((Math.random() * 10) % 2), startDate.getMinutes() + Math.max(30, Math.round((Math.random() * 100) % 59)), 0, 0);

			const progress = i <= completedTickets - 1 ? 100 : Math.round((random * 1000) % 100),
				duration = ((endDate.getTime() - startDate.getTime()) / (60 * 1000)),
				delay = (Math.random() > 0.5 ? Math.round((Math.random() * 100) % 60) : 0),
				loadType = loadTypes[Math.round(Math.random() * 10) % loadTypes.length],
				completionStatus = completionStatuses[Math.round(Math.random() * 20) % completionStatuses.length],
				transmissionStatus = transmissionStatuses[Math.round(Math.random() * 20) % transmissionStatuses.length],
				inProgress = i === activeTicketIndex ? 'Y' : 'N';
			let containerSize = '';

			if (loadType.indexOf('S/O') > -1 || loadType.indexOf('DEL') > -1) {
				containerSize = ' ' + containerSizes[Math.round(Math.random() * 10) % (containerSizes.length - 1)];
			}

			//Avoid duplicate ids
			while (orderIds.indexOf(id) > -1) {
				id = Math.round(Math.random() * count * 10);
			}
			...
			const ticket = {
				customerOrderId: id,
				customerName: customers[Math.round((Math.random() * 10)) % (customers.length - 1)],
				ticketStart: `${startDate.getHours()}:${startDate.getMinutes()}`,
				ticketEnd: `${endDate.getHours()}:${endDate.getMinutes()}`,
				duration: duration - delay < 0 ? duration : (duration - delay), //actual duration in minutes
				progress: progress,
				completed: i >= completedTickets ? 'N' : 'Y',
				inProgress: inProgress,
				hotTicket: i % 3 === 0 ? 'Y' : 'N',
				serviceStartWindow: `${serviceWindowStart.getHours()}:${serviceWindowStart.getMinutes()}`,
				serviceEndWindow: `${serviceWindowEnd.getHours()}:${serviceWindowEnd.getMinutes()}`,
				priorityCustomer: i % 2 === 0 ? 'Y' : 'N',
				loadType: loadType + containerSize,
				currentStatus: completionStatus,
				//disposalTime: '?',
				//containerYardTime: '?',
				//containerYardPresent: '?',
				//disposalTripPresent: '?',
				transmissionStatus: transmissionStatus //Ticket completion status '?'
			};

			if (i > count / 2 && !lunchBreak) {
				const minutes = Math.max(30, Math.round((Math.random() * 100) % 60));

				lunchBreak = true;
				tickets.push(getTicket('lunchTime', startDate, minutes));

				startDate.setMinutes(startDate.getMinutes() + minutes);
			}
			else if (i > count / 3 && !returnYard) {
				const minutes = Math.max(30, Math.round((Math.random() * 100) % 60));

				returnYard = true;
				tickets.push(getTicket('returnYard', startDate, minutes));
		...
		tickets.push(getTicket('returnToYard', startDate, minutes = Math.max(30, Math.round((Math.random() * 100) % 45))));

		startDate.setMinutes(startDate.getMinutes() + minutes);

		tickets.push(getTicket('postRoute', startDate, Math.max(30, Math.round((Math.random() * 100) % 45))));

		return tickets
	}

	function getDataSource(count = 10) {
		let data = [],
			// ids = [];
			toTwoDigits = (number) => {
		...
		for (let i = 0; i < count; i++) {
			const random = Math.random(),
				ticketsCount = Math.max(4, Math.round(Math.random() * 20)),
				completedTickets = Math.round(random * Math.pow(10, ticketsCount.toString().length) % ticketsCount);
			// let id = Math.round(random * 10000);
			let drhv = random >= 0.5 ? parseFloat((Math.random() * 5).toFixed(2)) : 0;
			const dynamicRouteHoursVariance = (drhv > 0 ? (random <= 0.7 ? '-' : '+') : '') + drhv + (drhv < 1 ? 'm' : 'h'),
				tickets = getTickets(ticketsCount, completedTickets);
			...
			date.setHours((random * 100) % 23, (random * 100) % 59);
			data.push({
				routeOrderId: i, //id,
				route: getRouteNumber(),
				driver: `${lastNames[Math.round((random * 100) % (lastNames.length - 1))]}, ${firstNames[Math.round((Math.random() * 100) % (firstNames.length - 1))]}`,
				vehicle: Math.round(random * 1000000) + '',
				//currentTime: '?',
				ticketsCompleted: `${Math.round(random * Math.pow(10, ticketsCount.toString().length) % ticketsCount)}/${ticketsCount}`,
				dynamicRouteHours: `${toTwoDigits(date.getHours())}:${toTwoDigits(date.getMinutes())}`,
				dynamicRouteHoursVariance: dynamicRouteHoursVariance,
				efficiency: parseFloat(random.toFixed(2)),
				haulMatchScore: Math.round((random * 1000) % 100) + '%',
				//routeType: '?',
				routeOrderData: {
					'actual': {
						'customerOrderData': tickets
					},
					'planned': {
						'customerOrderData': tickets
					}
...
export function updateDriverStatus(dataSource) {
	const transmissionStatuses = ['transmit_issue', 'transmit_ok', 'transmit_going', 'transmit_failed', 'recall', 'recall_ok', 'recall_fail', 'no_status'];
	let driverStatuses = [];

	for (let i = 0, max = dataSource.length; i < max; i += 1) {
		const dataItem = dataSource[i],
			routeOrderId = dataItem.routeOrderId,
			tickets = dataItem.routeOrderData.planned.customerOrderData;
			...
			routeOrderData: {
				'planned': {
					'customerOrderData': (() => {
						...
						for (let i = 0, max = tickets.length; i < max; i += 1) {
							const ticketData = tickets[i],
								orderId = ticketData.customerOrderId;
